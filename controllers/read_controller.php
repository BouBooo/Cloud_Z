...
$db = Database::connect();
$msg = $db->prepare("SELECT m.id, m.id_expediteur, m.id_destinataire, m.message, mem.email FROM messages m JOIN membres mem ON m.id_destinataire = mem.id WHERE m.id = ? AND m.id_destinataire = ?");
